    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h2 class="font-semibold text-xl text-gray-800 mb-6">Edit Contact: {{ $contact->name }}</h2>
                ...
                <form method="post" action="{{ route('admin.outreach.update', $contact) }}" class="space-y-6" x-data="{ personaType: '{{ old('persona_type', $contact->persona_type) }}' }">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="email" id="email" name="email" value="{{ old('email', $contact->email) }}" placeholder="email@example.com"
                                class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-[#16a34a] focus:ring-[#16a34a]">
                        </div>
                        <div>
                            <label for="linkedin_url" class="block text-sm font-medium text-gray-700 mb-1">LinkedIn URL</label>
                            <input type="url" id="linkedin_url" name="linkedin_url" value="{{ old('linkedin_url', $contact->linkedin_url) }}" placeholder="https://linkedin.com/in/..."
                                class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-[#16a34a] focus:ring-[#16a34a]">
                    ...
                    <div>
                        ...
                        <select id="persona_type" name="persona_type" required
                            ...
                            @foreach($personaTypes as $value => $label)
                                <option value="{{ $value }}" {{ old('persona_type', $contact->persona_type) === (string) $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div x-show="personaType === 'operator'" x-cloak x-transition class="space-y-1">
                        ...
                        <select id="operator_type" name="operator_type" :required="personaType === 'operator'"
                            ...
                            @foreach($operatorTypes as $value => $label)
                                <option value="{{ $value }}" {{ old('operator_type', $contact->operator_type) === (string) $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="organization" name="organization" value="{{ old('organization', $contact->organization) }}"
                            class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-[#16a34a] focus:ring-[#16a34a]">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="why_selected" name="why_selected" rows="2" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-[#16a34a] focus:ring-[#16a34a]">{{ old('why_selected', $contact->why_selected) }}</textarea>
                    </div>
                        ...
                        <div>
                            ...
                            <select id="priority" name="priority" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-[#16a34a] focus:ring-[#16a34a]">
                                ...
                                @foreach($priorities as $p)
                                    <option value="{{ $p }}" {{ old('priority', $contact->priority) === $p ? 'selected' : '' }}>{{ $p }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <select id="status" name="status" required class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-[#16a34a] focus:ring-[#16a34a]">
                            @foreach($statuses as $s)
                                <option value="{{ $s }}" {{ old('status', $contact->status) === $s ? 'selected' : '' }}>{{ $s }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" id="date_contacted" name="date_contacted" value="{{ old('date_contacted', $contact->date_contacted?->format('Y-m-d')) }}"
                                class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-[#16a34a] focus:ring-[#16a34a]">
                        </div>
                        <div>
                            <label for="follow_up_date" class="block text-sm font-medium text-gray-700 mb-1">Follow-up date</label>
                            <input type="date" id="follow_up_date" name="follow_up_date" value="{{ old('follow_up_date', $contact->follow_up_date?->format('Y-m-d')) }}"
                                class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-[#16a34a] focus:ring-[#16a34a]">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="response_summary" name="response_summary" rows="3" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-[#16a34a] focus:ring-[#16a34a]">{{ old('response_summary', $contact->response_summary) }}</textarea>
                    </div>

                    <div>
                        <label for="notes" class="block text-sm font-medium text-gray-700 mb-1">Notes</label>
                        <textarea id="notes" name="notes" rows="3" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-[#16a34a] focus:ring-[#16a34a]">{{ old('notes', $contact->notes) }}</textarea>
                    </div>
